...
export class ProgressionManager {
  constructor(scene) {
    this.scene = scene;
    this.loreManager = new LoreManager(scene);
  }
  ...
  nextLevel() {
    ...
    const r = Math.random();
    let nextNode;
    if (r < 0.40) nextNode = 'combat';
    else if (r < 0.475) nextNode = 'elite';
    else if (r < 0.55) nextNode = 'treasure';
    else if (r < 0.65) nextNode = 'shop';
    else nextNode = 'event';
    ...
    if (scene.isBossLevel || (scene.nodeType === 'elite' && nextNode !== 'shop' && nextNode !== 'event')) {
      ...
      const loreEvent = this.loreManager.getNextEvent(nextLevelNumber);
      if (loreEvent) {
        scene.registry.set('loreEvent', loreEvent);
      } else {
        scene.registry.set('loreEvent', null);
      }
